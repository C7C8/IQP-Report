...
\usepackage{listofauthorships}                      % CUSTOM BUILT (lifted from S/O)! Adds a table of authorships
...
%including allowing emergency vehicles quicker patient travel times.
...
% \paragraph{DRSC enabled vehicles} Dedicated short-range communication vehicles using surroundings to inform drivers and increase safety
...
% Tangible goals or measurements:
% 	Residents: 
% 		at most one vehicle per household
% 	Tourists:
% 		use other methods of transportation (not car rental or drive down to Eilat)
%     Establish a threshold and have traffic be below it at all times (including peak times)
% Addressing it:
% 	Policy
%       parking
%   	  zoning
...
% Tangible goals or measurements:
% 	Transportation is accessible (can get from any point of the city to another without using a privately owned vehicle within 20(?) minutes)
%     Transportation should be affordable (determine what price range would be appropriate according to Eilat's population)
%     various service are offered (other than buses and taxis)
%   Ease of use
% Addressing it:
% 	last mile services
% 	ride sharing
